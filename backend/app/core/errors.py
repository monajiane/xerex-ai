...
class BootstrapDisabledError(AppError):
    """First-run setup is switched off for this deployment."""

    status_code = status.HTTP_403_FORBIDDEN
    code = "bootstrap_disabled"
    message = "Initial setup is disabled on this deployment."


class RoleEscalationError(AppError):
    """The actor tried to grant a role they are not allowed to grant."""

    status_code = status.HTTP_403_FORBIDDEN
    code = "role_escalation_not_allowed"
    message = "Only an owner can grant the owner role."


class SelfRoleChangeError(AppError):
...
def register_exception_handlers(app: FastAPI) -> None:
    ...
    async def _validation_error(request: Request, exc: RequestValidationError) -> JSONResponse:
        if request.url.path.startswith("/v1"):
            # The public gateway is OpenAI-compatible on the way in too: a malformed
            # body is reported with the OpenAI error shape, not the admin envelope.
