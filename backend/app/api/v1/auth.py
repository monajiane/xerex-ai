...
async def bootstrap_status(session: SessionDep) -> BootstrapStatus:
    ...
    # ``bootstrap_enabled`` reports the effective switch: an explicit
    # XEREX_BOOTSTRAP_ENABLED wins, otherwise it follows the environment (enabled in
    # development/test, disabled in staging/production).
    enabled = settings.bootstrap_enabled_effective
    return BootstrapStatus(
        requires_bootstrap=count == 0,
        bootstrap_enabled=enabled,
        admin_user_count=count,
        bootstrap_allowed=enabled and count == 0,
...
async def login(
    ...
    limit = await login_limiter.hit(f"{ip}:{payload.email.lower()}")
