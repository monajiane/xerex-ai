...
describe("admin panel smoke test", () => {
  ...
  it("shows the sign-in screen with an explanation when first-run setup is disabled", async () => {
    mockApi({
      "/auth/bootstrap-status": {
    ...
    expect(await screen.findByText("ورود به پنل مدیریت")).toBeInTheDocument();
    expect(
      screen.getByText(/راه‌اندازی اولیه روی این استقرار غیرفعال است/),
    ).toBeInTheDocument();
    expect(screen.queryByRole("button", { name: /ساخت حساب مدیر/ })).not.toBeInTheDocument();
  });
