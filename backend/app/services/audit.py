...
class AuditService:
    ...
    async def record(
        ...
        *,
        ...
        ``diff`` is redacted recursively before it is stored, so a caller that
        passes a provider secret or an API key by mistake cannot persist it in the
        audit trail (PROMPT.md 7 — no plaintext credentials in audit logs).
        """
        safe_diff = redact(diff) if diff is not None else None
        entry = AuditLog(
